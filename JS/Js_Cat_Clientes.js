...
function eventos() {
    ...
    $('#btn_guardar').on('click', function (e) {
        ...
        if (output.Estatus) {
            if ($('#txt_cliente_id').val() != null && $('#txt_cliente_id').val() != undefined && $('#txt_cliente_id').val() != '') {
                Ope_Modificar();
            } else {
                Ope_Alta();
            }
...
function cargar_tabla() {
    var registros = "{}";
    $.ajax({
        url: 'Controllers/Ctrl_Clientes.asmx/Consultar',
        method: 'POST',
        cache: false,
        async: true,
        responsive: true,
        ...
        success: function (data) {
            ...
            oTable = $('#Tbl_Registros').DataTable({
                ...
                columns: [
                    ...
                    { title: "Cliente_ID", visible:false },
                    { title: "Bodega", visible: false },
...
function Ope_Alta() {
    ...
    try {
        Obj_usuario.Nombre = $('#txt_nombre').val();
        Obj_usuario.Cajas_Pendientes = "0";
        Obj_usuario.Cuentas_Pendientes = "0";

        if ($('#chk_Bodega').prop('checked')) {
            Obj_usuario.Bodega = "S";
        }
        else {
            Obj_usuario.Bodega = "N";
        }

        $.ajax({
            url: 'Controllers/Ctrl_Clientes.asmx/Ope_Alta',
...
function Ope_Modificar() {
    ...
    try {
        ...
        Obj_usuario.Nombre = $('#txt_nombre').val();
        Obj_usuario.Cajas_Pendientes = $('#txt_cajas').val();
        Obj_usuario.Cuentas_Pendientes = $('#txt_cuentas').val();
        ...
        $.ajax({
            url: 'Controllers/Ctrl_Clientes.asmx/Ope_Modificar',
            data: "{'Datos':'" + JSON.stringify(Obj_usuario) + "'}",
            type: 'POST',
            cache: false,
            async: true,
...
function Cargar_Informacion(row) {
    ...
    $('#txt_cajas').val(res[1]);
    $('#txt_cuentas').val(res[2]);
    $('#txt_cliente_id').val(res[3]);

    if (res[4] == 'S') {
        $("#chk_Bodega").prop("checked", true);
    }
    else {
        $("#chk_Bodega").prop("checked", false);
    }
