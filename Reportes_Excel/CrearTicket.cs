...
namespace JPV_Portal.Reportes_Excel
{
    public class CrearTicket
    {
            ...
            public void EncabezadoVenta()
            {
                ...
                linea.AppendLine("CANTID|ARTICULO        |PRECIO | IMPORTE");
            }
            public void EncabezadoVenta_cajas()
            {
                //Escribimos los espacios para mostrar el articulo. En total tienen que ser 40 caracteres
                linea.AppendLine("CANTID|ARTICULO                        ");
            }
        ...
        public void TextoIzquierda(string texto)
            ...
            public void AgregaArticulo(decimal cant, string articulo, decimal precio, decimal importe)
            {
                //Valida que cant precio e importe esten dentro del rango.
                if (cant.ToString().Length <= 6 && precio.ToString().Length <= 7 && importe.ToString().Length <= 8)
                {
                    string elemento = "", espacios = "";
                    string elemento_cant = "", espacios_cant = "";
                    ...
                    if (articulo.Length > 15)
                    {
                        ...
                        for (int i = 0; i < nroEspacios; i++)
                        {
                            espacios_cant += " ";//Generamos los espacios necesarios para alinear a la derecha
                        }
                        elemento_cant += espacios_cant + cant.ToString();//agregamos la cantidad con los espacios

                        //Colocar el precio a la derecha.
                        nroEspacios = (10 - precio.ToString().Length);
                        espacios = "";
                        for (int i = 2; i < nroEspacios; i++)
                        {
                            espacios += " ";//Genera los espacios
                        }
                        //el operador += indica que agregar mas cadenas a lo que ya existe.
                        elemento += espacios + String.Format("{0:0.0}", precio);//Agregamos el precio a la variable elemento
                        ...
                        elemento += " " + espacios + String.Format("{0:0.0}", importe);//Agregamos el importe alineado a la derecha

                        int caracterActual = 0;//Indicara en que caracter se quedo al bajae a la siguiente linea

                        //Por cada 18 caracteres se agregara una linea siguiente
                        for (int longitudTexto = articulo.Length; longitudTexto > 15; longitudTexto -= 15)
                        {
                            if (bandera == false)//si es false o la primera linea en recorrerer, continuar...
                            {
                                ...
                                linea.AppendLine(elemento_cant + " " + articulo.Substring(caracterActual, 15) + elemento);
                            ...
                            caracterActual += 15;//incrementa en 18 el valor de la variable caracterActual
                        }
                    ...
                    else //Si no es mayor solo agregarlo, sin dar saltos de lineas
                    {
                        ...
                        if (nroEspacios == 0)
                        {
                            ...
                            {
                                espacios += " "; //Agrega espacios para completar los 18 caracteres
                            }
                            elemento += " " + articulo + espacios + "   ";
                        }
                        else if (nroEspacios == 1)
                        {
                            for (int i = 0; i < (15 - articulo.Length); i++)
                            {
                                espacios += " "; //Agrega espacios para completar los 18 caracteres
                        ...
                        if (nroEspacios == 5)
                        {
                            ...
                            espacios = "";
                            for (int i = 4; i < nroEspacios; i++)
                            {
                                espacios += " ";
                            }
                            elemento += espacios + String.Format("{0:0.0}", precio);
                        }
                        else if (nroEspacios == 4)
                        {
                            nroEspacios = (7 - precio.ToString().Length);//7
                            espacios = "";
                            for (int i = 3; i < nroEspacios; i++)
                            {
                                espacios += " ";
                            }
                            elemento += espacios + String.Format("{0:0.0}", precio);
                        ...
                        elemento += " " +espacios+String.Format("{0:0.0}", importe);
        ...
        //**********************************************************imagen****************************************
        //private void imprime_imagen(object Imagen, PrintPageEventArgs e)
        //{
        //    Pen myPen = new Pen(Color.Black);

        //        Rectangle border = new Rectangle(20, 60, 810, 180);
        //        e.Graphics.DrawRectangle(Pens.Black, border);
        //        e.Graphics.DrawImage(Imagen.Image, 30, 80, this.pictureBox1.Size.Width, this.pictureBox1.Size.Height);
        //}
        //***************************************************************************************************************
        //Metodo para agreagar articulos al ticket de venta
        public void AgregaArticulo_Caja(decimal cant, string articulo)
        {
            //Valida que cant precio e importe esten dentro del rango.
            if (cant.ToString().Length <= 6)
            {
                string elemento = "", espacios = "";
                string elemento_cant = "", espacios_cant = "";
                bool bandera = false;//Indicara si es la primera linea que se escribe cuando bajemos a la segunda si el nombre del articulo no entra en la primera linea
...
//*************************************************************************************************************
        ...
        public void CortaTicket()
            ...
            //imprimir logo
            public void Imprimir_Logo()
            {
                linea.AppendLine("chr(29)+chr(47)+chr(0)+chr(48)"); //Caracteres de logo. Estos comando varian segun el tipo de impresora
                //linea.AppendLine("\x1B" + "d" + "\x09"); //Avanza 9 renglones, Tambien varian
            }
            //Para abrir el cajon
